...
async def ping(ctx):
    await ctx.respond(f'{round(client.latency * 1000)}ms')
...
async def nol(ctx, card):
    ...
    if nol_details['Error'] is False:
        embed = discord.Embed(title=f"NOL Details",
                              description=f"{nol_details['NolID']}")
        embed.set_thumbnail(
        ...
        embed.add_field(name="Card Balance",
                        value=f"{nol_details['Card Balance']}")
        embed.add_field(name="Pending Balance",
                        value=f"{nol_details['Pending Balance']}")
        embed.add_field(name="Expiry Date",
                        value=f"{nol_details['Expiry Date']}")
        embed.set_footer(
            text=f"Please note that the shown balance may not include transactions occurred in the past 48 hours")
...
async def recent(ctx, card, transaction=1):
    ...
    if recent['Error'] is False:
        if transaction == 1:
            embed = discord.Embed(
                title=f"Last NOL Transaction", description=f"{recent['NolID']}")
        else:
            embed = discord.Embed(
                title=f"Recent NOL Transaction Number {transaction}", description=f"{recent['NolID']}")
        embed.set_thumbnail(
            url="https://www.rta.ae/wps/wcm/connect/rta/3ae021ee-ea75-4c10-a579-35ab58bcf20d/apps.png?MOD=AJPERES&CACHEID=ROOTWORKSPACE.Z18_N004G041LOBR60AUHP2NT32000-3ae021ee-ea75-4c10-a579-35ab58bcf20d-nUKFITN")
        ...
        embed.set_footer(
            text=f"Please note that the bot only shows transactions occurred in the past month")
        await ctx.reply(embed=embed)
    else:
...
async def bal(ctx, card: discord.Option(str, "NOL Card Number", requied=True)):
    nolbal = Nol.Details(card)
    if nolbal['Error'] is False:
        await ctx.respond(f"Your Nol card balance is : {nolbal['Balance']}", ephemeral=True)
    else:
        await ctx.respond(f"{card} is not a valid NOL Card", ephemeral=True)
